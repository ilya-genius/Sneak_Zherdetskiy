...
const output = `Игра реализована с использованием HTML Canvas для отображения графики и JavaScript для логики игры. 
Она позволяет игроку управлять змейкой, собирать еду и набирать очки, при этом сталкиваясь с различными уровнями сложности и препятствиями.
Кастомные функции:
startGame(): Эта функция вызывается при начале игры после того, как пользователь ввел свой никнейм и выбрал уровень сложности. 
Она запускает основной игровой процесс.
toggleSound(): Эта функция переключает звук в игре. При каждом вызове она меняет иконку звука, а также включает или выключает звук в игре.
pausePlay(): Эта функция позволяет приостановить или возобновить игру по нажатию на кнопку "Esc".
saveScore(): Эта функция сохраняет результаты игры в локальном хранилище браузера. 
Она получает имя игрока и его счет, добавляет или обновляет запись в таблице рекордов и обновляет отображение таблицы на странице.
updateScoreboard(): Эта функция обновляет таблицу рекордов на странице, отображая последние результаты игры. 
Она получает данные из локального хранилища и отображает их в соответствующем формате.`;
console.log(output);